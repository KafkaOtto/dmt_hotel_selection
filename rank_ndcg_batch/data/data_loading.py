...
from rank_ndcg_batch.utils.common import load_train_eval_test_df, load_df_to_features
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        """
        x_num, x_cat, q, y, indices = sample
        return torch.from_numpy(x_num).type(torch.float32), torch.from_numpy(x_cat).type(torch.float32), torch.from_numpy(q).type(torch.long), torch.from_numpy(y).type(torch.float32), torch.from_numpy(indices).type(torch.long)
...
class FixLength(object):
    ...
    def __call__(self, sample):
        ...
        """
        sample_size = len(sample[2])
        if sample_size < self.dim_given:  # when expected dimension is larger than number of observation in instance do the padding
            fixed_len_x_num, fixed_len_x_cat, fixed_len_query_extra, fixed_len_y, indices = self._pad(sample, sample_size)
        else:  # otherwise do the sampling
            fixed_len_x_num, fixed_len_x_cat, fixed_len_query_extra, fixed_len_y, indices = self._sample(sample, sample_size)
        return fixed_len_x_num, fixed_len_x_cat, fixed_len_query_extra, fixed_len_y, indices
    ...
    def _sample(self, sample, sample_size):
        ...
        fixed_len_y = sample[3][indices]
        if fixed_len_y.sum() == 0:
            if sample[3].sum() == 1:
                indices = np.concatenate([np.random.choice(indices, self.dim_given - 1, replace=False), [np.argmax(sample[3])]])
                fixed_len_y = sample[3][indices]
            elif sample[3].sum() > 0:
                return self._sample(sample, sample_size)
        fixed_len_x_num = sample[0][indices]
        fixed_len_x_cat = sample[1][indices]
        fixed_len_query_extra = sample[2][indices]
        return fixed_len_x_num, fixed_len_x_cat, fixed_len_query_extra, fixed_len_y, indices
    ...
    def _pad(self, sample, sample_size):
        ...
        :return: ndarrays tuple containing features, labels and original ranks of shapes
            [sample_size, features_dim], [sample_size] and [sample_size]
        """
        fixed_len_x_num = np.pad(sample[0], ((0, self.dim_given - sample_size), (0, 0)), "constant")
        fixed_len_x_cat = np.pad(sample[1], ((0, self.dim_given - sample_size), (0, 0)), "constant")
        fixed_len_query_extra = np.pad(sample[2], ((0, self.dim_given - sample_size), (0, 0)), "constant")
        fixed_len_y = np.pad(sample[3], (0, self.dim_given - sample_size), "constant", constant_values=PADDED_Y_VALUE)
        indices = np.pad(np.arange(0, sample_size), (0, self.dim_given - sample_size), "constant", constant_values=PADDED_INDEX_VALUE)
        return fixed_len_x_num, fixed_len_x_cat, fixed_len_query_extra, fixed_len_y, indices


class HotelSearchDataset(Dataset):
    """
    LibSVM Learning to Rank dataset.
    """
    def __init__(self, X_num, X_cat, y, query_ids, transform=None):
        """
        ...
        _, indices, counts = np.unique(query_ids[:, 0], return_index=True, return_counts=True)
        groups = np.cumsum(counts[np.argsort(indices)])

        self.X_num_by_qid = np.split(X_num, groups)[:-1]
        self.X_cat_by_qid = np.split(X_cat, groups)[:-1]
        self.extra_ids = np.split(query_ids, groups)[:-1]
        self.y_by_qid = np.split(y, groups)[:-1]

        self.longest_query_length = max([len(a) for a in self.X_num_by_qid])

        logger.info("loaded dataset with {} queries".format(len(self.X_num_by_qid)))
        logger.info("longest query had {} documents".format(self.longest_query_length))

        self.transform = transform

    @classmethod
    def from_csv_file(cls, df, transform=None):
        """
        ...
        """
        x_num, x_cat, y, query_ids = load_df_to_features(df)
        logger.info("loaded dataset and got x_num shape {}, x_cat shape {}, y shape {} and query_ids shape {}".format(
            x_num.shape, x_cat.shape, y.shape, query_ids.shape))
        return cls(x_num, x_cat, y, query_ids, transform)

    def __len__(self):
        """
        :return: number of groups (slates) in the dataset
        """
        return len(self.X_num_by_qid)
    ...
    def __getitem__(self, idx):
        ...
        """
        X_num = self.X_num_by_qid[idx]
        X_cat = self.X_cat_by_qid[idx]
        query_extra = self.extra_ids[idx]
        y = self.y_by_qid[idx]

        sample = X_num, X_cat, query_extra, y
    ...
    def shape(self):
        ...
        num_dim = self[0][0].shape[-1]
        cat_dim = self[0][1].shape[-1]
        return [batch_dim, document_dim, num_dim, cat_dim]


# def load_search_data_role(input_path: str, role: str, slate_length: int) -> HotelSearchDataset:
#     """
...
#     """
#     transform = transforms.Compose([FixLength(slate_length), ToTensor()])
#     if role == "train":
#         path = input_path + "train_data.csv"
#     else:
#         path = input_path + "test_data.csv"
#     logger.info("will load {} data from {}".format(role, path))
#     ds = HotelSearchDataset.from_csv_file(path, transform)
#     logger.info("{} DS shape: {}".format(role, ds.shape))
#     return ds


def fix_length_to_longest_slate(ds: HotelSearchDataset) -> Compose:
    """
...
def load_search_dataset(input_path: str, slate_length: int) \
        -> Tuple[HotelSearchDataset, HotelSearchDataset]:
    """
    ...
    """
    train_path = input_path + "training_set_VU_DM.csv"
    test_path = input_path + "test_set_VU_DM.csv"
    logger.info("will data from {}, {}".format(train_path, test_path))
    transform = transforms.Compose([FixLength(slate_length), ToTensor()])
    df_train, df_eval, df_test = load_train_eval_test_df(train_path, test_path)
    train_ds = HotelSearchDataset.from_csv_file(df_train, transform)
    val_ds = HotelSearchDataset.from_csv_file(df_eval, transform)
    test_ds = HotelSearchDataset.from_csv_file(df_test, transform)
    return train_ds, val_ds, test_ds

def create_data_loaders(train_ds: HotelSearchDataset, val_ds: HotelSearchDataset, test_ds: HotelSearchDataset, num_workers: int, batch_size: int):
    """

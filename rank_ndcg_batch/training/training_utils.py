...
import rank_ndcg_batch.models.metrics as metrics_module
from rank_ndcg_batch.data.data_loading import PADDED_Y_VALUE
from rank_ndcg_batch.utils.model_utils import get_num_params, log_num_params
from rank_ndcg_batch.training.early_stop import EarlyStop
from rank_ndcg_batch.utils.logging_r import get_logger
from rank_ndcg_batch.utils.tensorboard_utils import TensorboardSummaryWriter

logger = get_logger()


def loss_batch(model, loss_func, xnumb, xcatb, yb, indices, gradient_clipping_norm, opt=None):
    mask = (yb == PADDED_Y_VALUE)
    y_pred = model(xnumb, xcatb, mask, indices)
    ...
    return loss.item(), len(xnumb)


def metric_on_batch(metric, model, xnumb, xcatb, yb, indices):
    mask = (yb == PADDED_Y_VALUE)
    return metric(model.score(xnumb, xcatb, mask, indices), yb)


def metric_on_epoch(metric, model, dl, dev):
    metric_values = torch.mean(
        torch.cat(
            [metric_on_batch(metric, model, xnumb.to(device=dev), xcatb.to(device=dev), yb.to(device=dev), indices.to(device=dev))
             for xnumb, xcatb, q, yb, indices in dl]
        ), dim=0
    ).cpu().numpy()
...
def fit(epochs, model, loss_func, optimizer, scheduler, train_dl, valid_dl, config,
    ...
    for epoch in range(epochs):
        ...
        train_losses, train_nums = zip(
            *[loss_batch(model, loss_func, xnumb.to(device=device), xcatb.to(device=device), yb.to(device=device), indices.to(device=device),
                         gradient_clipping_norm, optimizer) for
              xnumb, xcatb, q, yb, indices in train_dl])
        ...
        with torch.no_grad():
            val_losses, val_nums = zip(
                *[loss_batch(model, loss_func, xnumb.to(device=device), xcatb.to(device=device), yb.to(device=device), indices.to(device=device),
                  ...
                  xnumb, xcatb, q, yb, indices in valid_dl])
